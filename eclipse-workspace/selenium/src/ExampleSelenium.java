...
//				int hedsize = header.size();
				...
				for(int a=2,c=2;a<=3;a++,c++) {
					WebElement headerG = driver.findElement(By.xpath("(//span[@class='title-clr'])["+a+"]"));
					String hd1 = headerG.getText();
					XSSFRow row1 = sheet.createRow(0);
					XSSFCell cell1 = row1.createCell(c);  
					cell1.setCellValue(hd1);
				...
				for(int i=1;i<=hdS;i++) {
					...
					WebElement silverGrm1 = driver.findElement(By.xpath("(//font[@color='#484848' and text()>=50 and text()<70])["+i+"]"));
					String value1 = silverGrm1.getText();
					WebElement silver1Kg = driver.findElement(By.xpath("(//font[@color='#484848' and text()>=50000 and text()<70000])["+i+"]"));
					String value2 = silver1Kg.getText();
					System.out.println(value+ "|" +value1 +"|"+value2+"|");
				}
				FileOutputStream file = new FileOutputStream("E:\\gold.xlsx");
				 workbook.write(file);
				 workbook.close();
	}
}
